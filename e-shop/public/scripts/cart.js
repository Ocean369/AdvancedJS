...
const cartClose = {
    template: `
            <div class='cartUser-close'>
                <img src="image/close.svg" alt="close" @click='$root.$refs.cart.showCart = !$root.$refs.cart.showCart'>
            </div>
            `
};
                ...
                <div class='cartUser-body-item'>
                        ...
                        <div class=' cartUser-body_count'>
                            <input class='cartUser-body_countItem' type="number" min="1"
                                v-model.number='cartItem.quantity' @input='$root.$refs.cart.amount(cartItem)' />
                            pc.
                        </div>
                        <div>$ {{cartItem.price}}</div>
                        <div class='cartUser-body_amount'><span>$ {{cartItem.amount}}</span>
                            <img src="image/bin.png" alt="bin" @click='$root.$refs.cart.deleteFromCart(cartItem)'>
                        </div>
                </div>
    `
};
    ...
    methods: {
        addProduct(item) {
            ...
            if (find.length > 0) {
                ...
                this.amount(find[0]);
                this.$parent.putJson(`/api/cart`, find[0]);
